...
#'
#' @description Cancels a report in the Data Warehouse Queue
#'
#' @title Cancel a Report in the Data Warehouse Queue
...
CancelReport <- function(report.id) {
  ...
  request.body <- sprintf('{"Request_Id": %s}',report.id)
  ...
  if(AdobeAnalytics$SC.Credentials$auth.method=='OAUTH2') {
    ...
    url <- paste0(endpoint, '?method=DataWarehouse.CancelRequest')
